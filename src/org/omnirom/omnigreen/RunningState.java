...
public class RunningState {
    ...
    // entry.
    final SparseArray<HashMap<String, ProcessItem>> mServiceProcessesByName = new SparseArray<HashMap<String, ProcessItem>>();
    ...
    // If there are other users on the device, these are the merged items
    // representing all items that would be put in mUserBackgroundItems for that
    // user.
    ...
    final Comparator<RunningState.MergedItem> mBackgroundComparator = new Comparator<RunningState.MergedItem>() {
        @Override
        public int compare(MergedItem lhs, MergedItem rhs) {
            if (DEBUG_COMPARE) {
                Log.i(TAG, "Comparing " + lhs + " with " + rhs);
                Log.i(TAG, "     Proc " + lhs.mProcess + " with "
                        + rhs.mProcess);
            ...
            if (lhs.mProcess == rhs.mProcess) {
                ...
                return lhs.mLabel != null ? lhs.mLabel.compareTo(rhs.mLabel)
                        : -1;
            }
            ...
            if (rhs.mProcess == null)
                return 1;
            if (DEBUG_COMPARE)
                Log.i(TAG, "    Label " + lhs.mProcess.mLabel + " with "
                        + rhs.mProcess.mLabel);
    ...
    static class ProcessItem extends BaseItem {
        ...
        boolean updateService(Context context,
            ...
            if (si == null) {
                ...
                try {
                    si.mServiceInfo = ActivityThread.getPackageManager()
                            .getServiceInfo(service.service,
                                    PackageManager.GET_UNINSTALLED_PACKAGES,
                                    UserHandle.getUserId(service.uid));

                    if (si.mServiceInfo == null) {
                        Log.d("RunningService",
            ...
            if (service.clientPackage != null && service.clientLabel != 0) {
                ...
                try {
                    Resources clientr = pm
                            .getResourcesForApplication(service.clientPackage);
    ...
    static class MergedItem extends BaseItem {
        ...
        private void setDescription(Context context, int numProcesses,
                int numServices) {
            if (mLastNumProcesses != numProcesses
                ...
                if (numProcesses != 1) {
                    resid = numServices != 1 ? R.string.running_processes_item_description_p_p
                            : R.string.running_processes_item_description_p_s;
                } else if (numServices != 1) {
                    resid = R.string.running_processes_item_description_s_p;
                }
                mDescription = context.getResources().getString(resid,
        ...
        boolean update(Context context, boolean background) {
            ...
            if (mUser != null) {
                ...
                if (!mBackground) {
                    setDescription(context, (mProcess.mPid > 0 ? 1 : 0)
                            + mOtherProcesses.size(), mServices.size());
                }
    ...
    private RunningState(Context context) {
        ...
        mUm = (UserManager) mApplicationContext
    ...
    void updateNow() {
        update(mApplicationContext, mAm);
    }
    ...
    private void addOtherUserItem(Context context,
        ...
        if (first) {
            ...
            UserInfo info = userState != null ? userState.mInfo : mUm
                    .getUserInfo(newItem.mUserId);
            if (info == null) {
    ...
    private boolean update(Context context, ActivityManager am) {
        ...
        // Update state we are maintaining about process that are running
        // services.
        for (int i = 0; i < NS; i++) {
            ActivityManager.RunningServiceInfo si = services.get(i);
            ...
            if (si.restarting == 0 && si.pid > 0) {
                ...
                if (ainfo != null && !ainfo.hasForegroundServices) {
                    ...
                    if (ainfo.info.importance < ActivityManager.RunningAppProcessInfo.IMPORTANCE_SERVICE) {
                        ...
                        boolean skip = false;
                        ainfo = mTmpAppProcesses
                                .get(ainfo.info.importanceReasonPid);
                        while (ainfo != null) {
                            if (ainfo.hasServices
                                    || isInterestingProcess(ainfo.info)) {
                                skip = true;
                                break;
                            }
                            ainfo = mTmpAppProcesses
        ...
        if (newBackgroundItems != null) {
            ...
            // per-user items.
            if (!diffUsers) {
                // Easy: there are no other users, we can just use the same
                // array.
